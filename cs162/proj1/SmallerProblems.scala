...
object Problem1 {
    def main( args:Array[String] ) = {
	  ...
	  args.foreach(arg => print(arg.reverse + " "))
...
object Problem2 {
  ...
  def order( node:Node ) {
    ...
    def recursionPreOrder(node: Node, preorder: Int) : Int = {
	   var next = preorder
	   node.ord = node.ord match {case (x,y) =>  (next,y) case _ => throw new Exception("asd")}
	   next = next + 1
	   if (!node.left.isEmpty) next = recursionPreOrder(node.left.get, next)
	   if (!node.right.isEmpty) next = recursionPreOrder(node.right.get, next)
	   next
	}
	
	def recursionPostOrder(node: Node, postorder: Int) : Int = {
	   var next = postorder
	   if (!node.left.isEmpty) next = recursionPostOrder(node.left.get, next)
	   if (!node.right.isEmpty) next = recursionPostOrder(node.right.get, next)
	   node.ord = node.ord match {case (x,y) =>  (x, next) case _ => throw new Exception("asd")}
	   next +1
	}
	
	recursionPreOrder(node, 0)
	recursionPostOrder(node, 0)
...
object Problem3 {
  def main( args:Array[String] ) = {
	...
	println (list.foldLeft("")((x,y) => if (x>y) x else y))
	
	val noDul = list.reverse.foldLeft(List[String]())((x,y)=>(x.find(t=>t==y) match {case Some(_) => x case None => y::x}))
	
	println (noDul)
	
	
	println(noDul.zip( for (arg<-noDul) yield list.count(t=>t==arg)))
  }
}
