...
case class MyCons[A](x: A, xs: MyList[A]) extends MyList[A] {
  ...
  def map[B](f: A => B): MyList[B] = 
    if (xs.isEmpty) MyCons(f(x), MyNil[B]()) else MyCons(f(x), xs.map(f))

  def flatMap[B](f: A => MyList[B]): MyList[B] = {
    if (xs.isEmpty) f(x)
	else f(x).foldRight(xs.flatMap(f))(MyCons(_,_))
  }
    

  def filter(pred: A => Boolean): MyList[A] =
    if (pred(x)) MyCons(x, xs.filter(pred)) else xs.filter(pred)

  def append(other: MyList[A]): MyList[A] = {
    if (other.isEmpty) MyCons(x,xs)
	else MyCons(x,xs).foldRight(other)(MyCons(_,_))
  }

  def foldLeft[B](initial: B)(f: (B, A) => B): B = {
    //if (xs.isEmpty) f(initial, x) else f(xs.foldLeft(initial)(f), x)
  ...
  def foldRight[B](initial: B)(f: (A, B) => B): B = 
    if (xs.isEmpty) f(x, initial) else f(x, xs.foldRight(initial)(f))

  def take(n: Int): MyList[A] = {
    def res(n:Int, ins:MyList[A]) : MyList[A] = {
	  if (n <= 0) MyNil[A]
	  else if (ins.tail.isEmpty) MyCons(ins.head, MyNil[A])
	  else MyCons(ins.head, res(n-1,ins.tail))
	}
	res(n,MyCons(x,xs))
  }
  ...
  def drop(n: Int): MyList[A] = {
    def res(n:Int, ins:MyList[A]) : MyList[A] = {
	  ...
	  //println(ins)
	  if (n-1 <= 0) ins.tail
	  else if (ins.tail.isEmpty) MyNil[A]
	  else res(n-1, ins.tail)
	}
  ...
  def init: MyList[A] = {
    def res(ins:MyList[A]) : MyList[A] = {
	  if (ins.tail.isEmpty) MyNil[A]
	  else MyCons(ins.head,res(ins.tail))
	}
  ...
  def last: A = {
    def res(ins:MyList[A]) : A = {
	  if (ins.tail.isEmpty) ins.head
	  else res(ins.tail)
	}
  ...
  def length: Int = {
    def res(ins:MyList[A]) : Int = {
	  if (ins.tail.isEmpty) 1
	  else res(ins.tail) + 1
	}
...
case class MyNil[A]() extends MyList[A] {
  ...
  def head: A = throw new InvalidOperationException("No Head")

  def tail: MyList[A] = throw new InvalidOperationException("No Tail")

  def init: MyList[A] = throw new InvalidOperationException("No Init")

  def last: A = throw new InvalidOperationException("No Last")
