  ...
  def eval(goals: Seq[Closure], eq: Equiv): Unit =
    if (goals nonEmpty) goals.head match {
      ...
      case goalClo @ Closure(p: Predicate, env, as) =>
        {
		  ...
		  for (asn <- as) {
		    asn match {
			  case Fact(fv,fp) => {
			    //println("test1")
			    val newEq = matchPredicates(new Closure(fp,env++fv.map{case(v) => (v, LogicV())}.toMap,as), goalClo, eq)
				if (newEq != None) {
				  eval(goals.tail ,newEq get)
				}
			  }
			  case Clause(fv,fp,fg) => {
			    ...
			    val newEq = matchPredicates(new Closure(fp,env++aEnv,as), goalClo, eq)
				
				//println(env++fv.map{case(v) => (v, LogicV())}.toMap)
				//println(newEq.getOrElse(3))
				
				if (newEq != None) {
				  eval(new Closure(fg,env++aEnv,as)+:goals.tail ,newEq get)
				}
			  }
			}
		  }
		  //println("Predicate Exit")
		} // FILL ME IN
      ...
      case Closure(Conjunct(g1, g2), env, as) =>
        {
		  //println("Con")
		  eval(new Closure(g1,env,as)+:(new Closure(g2,env,as)+:goals.tail),eq)
		  //println("Con Exit")
		}// FILL ME IN
      ...
      case Closure(Disjunct(g1, g2), env, as) =>
        {
		  //println("Dis")
		  eval(new Closure(g1,env,as)+:goals.tail ,eq)
		  eval(new Closure(g2,env,as)+:goals.tail ,eq)
		} // FILL ME IN
      ...
      case Closure(Hypothetical(a, g), env, as) =>
        {
		  //println("Hypo")
		  eval(new Closure(g,env ,a+:as)+:goals.tail ,eq)
		  //++ v.map{case(v) => (v, LogicV())}.toMap
      ...
      case Closure(Exists(vars, g), env, as) =>
        {
		  //println("Exi")
		  eval(new Closure(g, env ++ vars.map{case(v) => (v, LogicV())}.toMap,as)+:goals.tail, eq)
		  //println("Exi exit")
		} // FILL ME IN
