...
solve(Board) :-
        innitial(Board, Cols, Cubes,0),
        fillNextGrid(Board,Cols,Cubes,0).


getNElement([L|_], 0, L).
getNElement([_|R], Num, L):-
       Num > 0,
	   NewNum is Num - 1,
	   getNElement(R,NewNum, L).
	   

innitial(Board ,Cols, Cubes,9) :-
        Cubes_ = [],
		Cols = [].
		
innitial(Board, Cols, Cubes, Num):-
        NextNum is Num + 1,
        innitial(Board,RCols,RCubes,NextNum),
		
		columnAsList(Board, Num, HCol),
		Cols = [HCol|RCols],
		
		getCube(Board, Num,HCube),
        Cubes = [HCube|RCubes].
	   
	   
fillNextGrid(_,_,_,81).
fillNextGrid(Board, Cols,Cubes,ThisGrid):-

        ThisRow is ThisGrid // 9,
		ThisCol is ThisGrid rem 9,
		ThisCube is ThisGrid // 27 * 3 + ThisGrid rem 27 rem 9 // 3,
		
		
		
		getNElement(Board,ThisRow,AsList0),
		
		getNElement(Cols,ThisCol,AsList1),
		
		getNElement(Cubes,ThisCube,AsList2),
		
		getNElement(AsList0,ThisCol,S),
        digit(S),
		...
		NextGrid is ThisGrid + 1,
		fillNextGrid(Board,Cols,Cubes,NextGrid).
