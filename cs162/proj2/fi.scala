...
object Images {
  ...
  def lift1[A, B] (func: A => B)(x: Img[A]) : Img[B] = pt => func(x(pt))
  ...
  def lift2[A,B,C] (func: (A,B) => C) (xa: Img[A])(xb: Img[B]) : Img[C] = pt => func(xa(pt),xb(pt))
  ...
  def lift3[A,B,C,D] (func: (A,B,C) => D) (xa: Img[A])(xb: Img[B])(xc: Img[C]) : Img[D] = pt => func(xa(pt),xb(pt),xc(pt))
  ...
  val msk2clr = lift1((b:Boolean) => if (b) black else white) _
  ...
  val gray2clr = lift1((v: Value) => lerpClr(v,black,white)) _
...
object SpatialTransforms {
  ...
  def rotatePt(theta: Double) : TransformPt = pt => Point(pt.x * cos(theta) - pt.y * sin(theta), pt.y * cos(theta) + pt.x * sin(theta))
  ...
  def swirlPt (theta: Double) : TransformPt = pt => rotatePt(Utility.dist(pt) * (2*Math.PI/theta))(pt)

  // define a method named 'polarXf' that takes a TransformPlr as
  // argument and returns a TransformPt.
  // !!FILL ME IN
  
  def polarXf (func: TransformPlr) : TransformPt = pt => Utility.plr2pt(func(Utility.pt2plr(pt)))
  ...
  def translate[A](v: Vector) : Filter[A] = (func) => (pt => func(translatePt(Vector(-v.x,-v.y))(pt)))
  ...
  def scale[A](v: Vector) : Filter[A] = (func) => (pt => func(scalePt(Vector(1/v.x,1/v.y))(pt)))
  ...
  def rotate[A](theta: Double) : Filter[A] = (func) => (pt => func(rotatePt(-theta)(pt)))
  ...
  def swirl[A](theta: Double) : Filter[A] = (func) => (pt => func(swirlPt(-theta)(pt)))
}
...
object Masking {
  ...
  val emptyM = lift0[Boolean](false)
  ...
  val notM = lift1[Boolean, Boolean] (!_) _
  ...
  val intersectM = lift2[Boolean, Boolean, Boolean] (_&_) _
  ...
  val unionM = lift2[Boolean, Boolean, Boolean] (_|_) _
  ...
  val xorM = lift2[Boolean, Boolean, Boolean] (_^_) _
  ...
  val diffM = lift2[Boolean, Boolean, Boolean] ((x,y) => x&(!y)) _
}
