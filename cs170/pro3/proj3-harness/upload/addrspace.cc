...
AddrSpace::AddrSpace(const AddrSpace* other, PCB* newPCB)
{
    ...
    for (unsigned int i = 0; i < numPages; i++) { 
        ...
        // TODO: What if the page has been modfied since it was swapped
        if ((other->pageTable)[i].valid && (other->pageTable)[i].dirty) {
              (other->pageTable)[i].dirty = false;
              virtualMemoryManager->writeToSwap(
                       &machine->mainMemory[(other->pageTable)[i].physicalPage * PageSize],
                       PageSize,(other->pageTable)[i].locationOnDisk);
        }
        // into main memory? Will the line below have the desired effect?
...
//     pages.
//----------------------------------------------------------------------
TranslationEntry* AddrSpace::getPage(int pageTableIndex)
{
