...
// then, copy in the code and data segments into memory
    ...
    if (noffH.code.size > 0) {
        ...
        int byteRemain = noffH.code.size;
        int byteRead = 0;
        while (byteRemain > PageSize) {
            executable->ReadAt(diskBuffer, PageSize, noffH.code.inFileAddr + byteRead);
             for(i = 0; i < PageSize; i++) 
               machine->mainMemory[Translate(noffH.code.virtualAddr + byteRead + i)] = diskBuffer[i];
            byteRead+=PageSize;
            byteRemain -= PageSize;
        }
        executable->ReadAt(diskBuffer, byteRemain, noffH.code.inFileAddr + byteRead);
        for(i = 0; i < byteRemain; i++)
            machine->mainMemory[Translate(noffH.code.virtualAddr + byteRead + i)] = diskBuffer[i];

        //for (i = noffH.code.virtualAddr; i < noffH.code.size; i++)
        //    DEBUG('c', "VM:%i PM:%i %x %x\n", noffH.code.virtualAddr + i, Translate(noffH.code.virtualAddr + i), machine->mainMemory[Translate(noffH.code.virtualAddr + byteRead + i)]);
...
//                           noffH.code.size, noffH.code.inFileAddr);
    ...
    if (noffH.initData.size > 0) {
        ...
        int byteRemain = noffH.initData.size;
        int byteRead = 0;
        while (byteRemain > PageSize) {
            executable->ReadAt(diskBuffer, PageSize, noffH.initData.inFileAddr + byteRead);
            for(i = 0; i < PageSize; i++) 
               machine->mainMemory[Translate(noffH.initData.virtualAddr + byteRead + i)] = diskBuffer[i];
            byteRead+=PageSize;
            byteRemain -= PageSize;
        }
        executable->ReadAt(diskBuffer, byteRemain, noffH.initData.inFileAddr + byteRead);
        for(i = 0; i < byteRemain; i++)
            machine->mainMemory[Translate(noffH.initData.virtualAddr + byteRead + i)] = diskBuffer[i];
...
//                           noffH.initData.size, noffH.initData.inFileAddr);
        ...
        for (i = noffH.initData.virtualAddr; i < noffH.initData.size; i++)
            DEBUG('c', "%c", machine->mainMemory[Translate(noffH.initData.virtualAddr + i)]);
    }
